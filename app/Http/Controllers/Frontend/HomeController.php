...
class HomeController extends Controller
{
    public function index()
    {
        ...
        return view('Frontend.Home.home', ['categories' => $categories, 'blogs' => $blogs, 'feature_post' => $feature_post]);
    }
    public function readBlog($blogpost_id)
    {
        ...
        $user = User::all();
        return view('Frontend.BlogHome.readblog', ['blog' => $blog, 'poster' => $poster, 'category' => $category, 'categories' => $categories, 'comments' => $comments, 'users' => $user]);
    }
    public function exploreCategory($category_id)
    {
        $blogs =  DB::table('blogposts')->where('category_id', $category_id)->simplePaginate(4);
        $category = Category::find($category_id);
        return view('Frontend.BlogHome.categoryBasedBlogs', ['blogs' => $blogs, 'category' => $category]);
    }
}
